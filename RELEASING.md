...
4. It uploads every release file under a digest-addressed immutable R2 prefix.
5. It downloads the release through the public domain and verifies the pinned
   inventory, every file digest, and both signed manifests.
6. It opens a pull request that changes only
   `desktop/capsule-guest/release-pin.json`.
7. Review and merge that pull request when future Desktop releases should use
   the new Guest.
...
3. The macOS arm64 job downloads and verifies the pinned Guest, builds the VM
   helper, stages native resources, packages the ad-hoc-signed App, and runs the
   packaged node-pty smoke.
