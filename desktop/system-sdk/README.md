...
Calls keep Vercel usage, finish information, warnings, and sanitized errors. `providerMetadata.lamarck.invocationId` identifies the Host invocation. Tool writes retain the existing Guard evidence; Each observed language generation also retains a content-bearing `ai.turn` event under its initiating App workload. Custom HTTP headers and raw provider chunks are not exposed. Provider request/debug bodies, credentials, and CLI diagnostics do not cross the App channel. Calls are limited to eight per channel and 64 globally, with bounded event queues, a 60-second idle timeout, and a ten-minute deadline. Cancellation, disconnect, permission withdrawal, or source reconfiguration ends the corresponding work.

### Captured AI content

SDK initialization registers one official `ai@7.0.105` telemetry integration in each browser or Node realm. Install that pinned peer alongside `@lamarck/system`; ordinary `generateText` and `streamText` calls need no capture wrapper. One generation ID produces one event across model calls, tools and retries. Model/access-source attribution belongs to each invocation in the structured record.

A completed event means the content-generation/tool cycle reached the SDK's normal `onEnd`. Later structured-output validation can still reject the App's generation promise; it neither changes that event nor runs again for capture. Terminal generation failures and cancellation retain the observed prefix as `failed` or `interrupted`.

The event contains redacted input, answer text, and an always-externalized structured record. Resolve its ContentRef using `system.resolveContentRef`. Text through 8,192 UTF-8 bytes stays inline; longer text has a safe preview and a reference to the full retained text. Reasoning, credentials, provider debug/context objects, and binary bytes are excluded; file descriptors retain usable references. Official `recordInputs`/`recordOutputs` controls apply. Disabled telemetry or per-call integrations that replace the global collector do not produce a synthetic event.

Host stages selected records incrementally and Guard validates and durably publishes blobs before the event. Collection has explicit limits: eight turns per channel, 64 globally, depth 32, a conservative 2 MiB selected-value budget, 16 MiB queued writes per turn, and at most 32 simultaneous text/tool-input parts. Incomplete tool input is bounded to 512 KiB and a pending text redaction token to 1 MiB. A limit, malformed record, correlation defect, or storage failure reports a capture error and publishes no substitute event. Browser errors use `reportError`; Node errors use the existing workload stderr path. Telemetry errors do not necessarily reject the official generation promise. Capture completion may follow resolution of a stream's `.text` property.

This records observed content through Lamarck access, not arbitrary App provider calls. It does not create conversations, retain native CLI debug transcripts, or certify that generated claims are true.
